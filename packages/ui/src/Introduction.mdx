...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>アラート通知（scv() ショーケース）</td>
      <td>Info / Success / Warning / Error、閉じるボタン対応</td>
    </tr>
    <tr>
      <td>
        <strong>Badge</strong>
    ...
    <tr>
      ...
      <td>タグ選択（compoundVariants ショーケース）</td>
      <td>Filled / Outlined × 5色、削除ボタン対応</td>
    </tr>
    <tr>
      <td>
...
```bash
bun run storybook:snapshot          # スクリーンショット撮影
bun run storybook:reg:local  # main ブランチとの差分レポート生成
```
